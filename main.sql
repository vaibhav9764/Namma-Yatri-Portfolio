...
INSERT INTO trips_details(tripid,loc_from,searches,searches_got_estimate,searches_for_quotes,searches_got_quotes,customer_not_cancelled,driver_not_cancelled,otp_entered,end_ride) 
...
-- MySQL Database and Schema Created
...
SELECT SUM(driver_not_cancelled = 0) AS total_driver_cancelled FROM trips_details;
...
SELECT COUNT(*) AS driver_cancelled FROM trips_details WHERE driver_not_cancelled = 0;

-- Cancelled bookings by customer
SELECT COUNT(*) AS customer_cancelled FROM trips_details WHERE customer_not_cancelled = 0;

-- Average distance per trip
SELECT AVG(distance) AS avg_distance_per_trip FROM trips;
...
SELECT method, MAX(fare) AS highest_payment 
FROM trips 
JOIN payment ON trips.faremethod = payment.id 
GROUP BY method;
...
SELECT d.duration, COUNT(*) AS trip_count
FROM trips t
...
SELECT driverid, custid, COUNT(*) AS order_count 
FROM trips 
...
SELECT (SUM(customer_not_cancelled) * 100.0 / SUM(searches_got_quotes)) AS quote_to_booking_rate 
FROM trips_details;

-- Booking cancellation rate
SELECT (SUM(customer_not_cancelled = 0 OR driver_not_cancelled = 0) * 100.0 / COUNT(*)) AS booking_cancellation_rate 
FROM trips_details;
...
SELECT t.loc_from, d.duration, COUNT(*) AS trip_count 
FROM trips t
JOIN duration d ON t.duration = d.id 
GROUP BY t.loc_from, d.duration 
ORDER BY trip_count DESC 
LIMIT 1;
...
SELECT loc_from, COUNT(*) AS cancellation_count 
FROM trips_details 
WHERE customer_not_cancelled = 0 OR driver_not_cancelled = 0 
GROUP BY loc_from 
ORDER BY cancellation_count DESC 
LIMIT 1;

SELECT loc_from, COUNT(*) AS trip_count 
FROM trips 
GROUP BY loc_from 
ORDER BY trip_count DESC 
LIMIT 1;
